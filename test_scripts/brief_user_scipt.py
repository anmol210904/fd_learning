...
class User:
    ...
    def submit_mask_shares(self):
        ...
        print(f"\nUSER {self.user_id} - Original Mask Generated: {mask}")
        
        shares = self.shamir_handler.split_secret(mask, self.num_participants, self.num_participants)
        print(f"USER {self.user_id} - Shares Created (before encryption):")
        for i, share in enumerate(shares):
            print(f"  - Share for User {i+1}: {share}")
    ...
    def submit_all_final_data(self, decrypted_shares, mask):
        ...
        print(f"\nUSER {self.user_id} - Original Weights Generated: {[round(w, 4) for w in self.original_weights]}")
    ...
    def fetch_global_model(self):
        ...
        response.raise_for_status()
        global_model_encoded = response.json()['global_model_weights']
        if not global_model_encoded:
            print(f"USER {self.user_id} - Global model received was empty.")
            return
        global_model_decoded = [w / PRECISION_FACTOR for w in global_model_encoded]
        print(f"\nUSER {self.user_id} - Final Decoded Global Model Received: {[round(w, 4) for w in global_model_decoded]}")
...
def perform_action_with_retry(action_function, action_name):
    ...
    """
    print(f"\n---> Attempting to perform: {action_name}")
    while True:
        try:
            # Attempt to perform the action
            result = action_function()
            print(f"---> Successfully completed: {action_name}")
            return result
        except requests.exceptions.HTTPError as e:
            # Check if the error is a 'Wrong Window' error from the server
            if e.response.status_code == 400 and 'Wrong' in e.response.text:
                print(f"     Server not in correct window for '{action_name}'. Waiting 10 seconds to retry...")
                time.sleep(10)
            else:
                # If it's a different HTTP error, raise it
                print(f"     An unexpected HTTP error occurred for {action_name}: {e.response.text}")
                raise
        except requests.exceptions.RequestException as e:
            # If it's a network error, raise it
            print(f"     A network error occurred for {action_name}: {e}")
            raise

def main():
    """Orchestrates the simulation for a single user, synchronized with the server."""
    user = User(user_id=random.randint(100, 999))
    
    while True:
        print("\n\n=============== STARTING NEW ROUND ===============")
        try:
            # Each of these calls will now automatically wait and retry until successful
            ...
            print(f"\n\nAn error occurred: {e}. Restarting after a delay.")
            time.sleep(10)
